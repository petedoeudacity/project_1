...
class NEODatabase:
    ...
    def __init__(self, neos, approaches):
        ...
        link them together - after it's done, the `.approaches` attribute
        ...
        # You want a list of all close approaches that have the same
        # ._designation, then you can attach these to the relevant NEO
        # attribute based on the designation designation is KEY;
        # list of approach objects with that designation is the VALUE

        designation_approach_map = {}

        for a in approaches:
            if a._designation in designation_approach_map:
                designation_approach_map[a._designation].append(a)

            else:
                designation_approach_map[a._designation] = [a]

        for neo in self._neos:
            if neo.designation in designation_approach_map:
                neo.approaches = designation_approach_map[neo.designation]

        # linking NEO object to each close approach based on designation
        # approach.neo = each individual approach's 'neo' attribute
        # 'neo_designations' is a dictionary created just above
        # that maps designations to neo objects
        # 'approach._designation' references the approach object's
        # '._designation' attribute
    ...
    def query(self, filters=()):
        ...
        #  else, loop through approaches; w/respect to each approach,
        # loop through the filters -- track w/test variable...
        else:
            for approach in self._approaches:
                test = True
                for filter in filters:
                    if not filter(approach):
                        # if the approach does not pass the specific
                        # filter criterion, change tracker,
                        # break out of loop
                        test = False
                        break
