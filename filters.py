...
that is used by
the `query` method to generate a stream of `CloseApproach` objects
that match
all of the desired criteria. The arguments to `create_filters` are
provided by
...
class AttributeFilter:
    ...
    value, and
    calling the filter (with __call__) executes `get(approach)
    OP value` (in
    infix notation).

    Concrete subclasses can override the `get` classmethod to
    provide custom
    behavior to fetch a desired attribute from the given
    `CloseApproach`.
    """
    ...
    def __init__(self, op, value):
        ...
        (right-hand side)
        argument to the operator function. For example, an
        `AttributeFilter`
        with `op=operator.le` and `value=10` will, when called
        on an approach,
    ...
    def __repr__(self):
        ...
        return (f"{self.__class__.__name__}(op=operator.{self.op.__name__},
                                           f"value={self.value}")
#     subclasses of the AttributeFilter, to be used
#     in the create_filters function


class dateFilter(AttributeFilter):
    """Date filter sublcass."""
...
def create_filters(date=None, start_date=None, end_date=None,
    ...
    a value from the
    user's options at the command line. Each one corresponds to
    a different type
    ...
    if distance_max is not None:
        fil = distanceFilter(operator.le, distance_max)
        filters.append(fil)

    if velocity_min is not None:
        fil = velocityFilter(operator.ge, velocity_min)
        filters.append(fil)

    if velocity_max is not None:
        fil = velocityFilter(operator.le, velocity_max)
        filters.append(fil)

    if diameter_min is not None:
        fil = diameterFilter(operator.ge, diameter_min)
        filters.append(fil)

    if diameter_max is not None:
        fil = diameterFilter(operator.le, diameter_max)
        filters.append(fil)

    if hazardous is not None:
        fil = hazardousFilter(operator.eq, hazardous)
        filters.append(fil)
